import { ArrowsFullscreen, People } from './SimpleIcons';
import { Link } from 'react-router-dom';
import type { ApiRoom } from '../types';

interface RoomProps {
  room: ApiRoom;
}

export default function Room({ room }: RoomProps) {
  const { id, roomNumber, mainImage, floor, guestCapacity, description, pricePerNight } = room;

  return (
    <div className="bg-white shadow-2xl min-h-[500px] group">
      <div className="overflow-hidden">
        <img
          src={mainImage}
          alt={`Room ${roomNumber}`}
          className="group-hover:scale-110 transition-all duration-300 w-full h-[300px] object-cover"
        />
      </div>
      <div className="bg-white shadow-lg max-w-[300px] mx-auto h-[60px] -translate-y-1/2 flex justify-center items-center uppercase font-tertiary tracking-[1px] font-semibold text-base">
        <div className="flex justify-between w-[80%]">
          <div className="flex items-center gap-x-2">
            <div className="text-accent">
              <ArrowsFullscreen className="text-[15px]" />
            </div>
            <div className="flex gap-x-1">
              <div>Floor</div>
              <div>{floor}</div>
            </div>
          </div>
          <div className="flex items-center gap-x-2">
            <div className="text-accent">
              <People className="text-[18px]" />
            </div>
            <div className="flex gap-x-1">
              <div>Max people</div>
              <div>{guestCapacity}</div>
            </div>
          </div>
        </div>
      </div>
      <div className="text-center">
        <Link to={`/room/${id}`}>
          <h3 className="h3">{roomNumber} - {room.type}</h3>
        </Link>
        <p className="max-w-[300px] mx-auto mb-3 lg:mb-6">{description.slice(0, 56)}..</p>
      </div>
      <Link
        to={`/room/${id}`}
        className="btn btn-secondary btn-sm max-w-[240px] mx-auto duration-300"
      >
        Book now from ${pricePerNight}
      </Link>
    </div>
  );
}
